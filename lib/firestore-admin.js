...
export async function setSharedList(shareId, data) {
  const token = await getAccessToken();
  const fields = {};
  ...
  if (data.createdAt) fields.createdAt = toFirestoreValue(data.createdAt);

  const res = await fetch(`${BASE}/sharedLists/${shareId}`, {
    method: 'PATCH',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
      ...
      result[k] = (v.arrayValue.values || []).map((item) => {
        ...
        if (item.mapValue && item.mapValue.fields) {
          for (const [mk, mv] of Object.entries(item.mapValue.fields)) {
            ...
            else if (mv.doubleValue !== undefined) obj[mk] = mv.doubleValue;
