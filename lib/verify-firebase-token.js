...
//
// Uses the JWK endpoint (not the x509 cert endpoint): Web Crypto's
// importKey('jwk', …) consumes these directly. The x509 certs are NOT valid
// SPKI input, so importing them as 'spki' throws "Invalid SPKI input".
...
async function getPublicKeys() {
  ...
  cachedKeys = {};
  for (const key of data.keys || []) cachedKeys[key.kid] = key;
  return cachedKeys;
}
...
export async function verifyFirebaseToken(idToken) {
  ...
  // Verify signature with the matching JWK
  const keys = await getPublicKeys();
  const jwk = keys[headerRaw.kid];
